function [net, info] = g2denet_train_simplenn(net, imdb, getBatch_train, getBatch_val, varargin)

% This code is used to train Global Gaussian Distribution 
% Embeding Network built in simplenn structure. This file 
% is modified from code of BCNN (https://bitbucket.org/tsungyu/bcnn).

% INPUT
% net: Global Gaussian Distribution Embeding Network
...
% OUTPUT
% net: Global Gaussian Distribution Embeding Network after 
% fine-tuning
...
for epoch=start+1:opts.numEpochs
  ...
  if ~evaluateMode && (mod(epoch, 5)==0), save(modelPath(epoch), 'net', 'info') ; end
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if s==1, res = []; end
    res = vl_GlbGaussian_nn(net, im, dzdy, res, ...
                      'accumulate', s ~= 1, ...
                      'mode', evalMode, ...
