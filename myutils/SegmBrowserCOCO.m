...
classdef SegmBrowserCOCO    
    ...
    methods
        function obj = SegmBrowserCOCO(exp_dir, mask_type, imgset, overlap_type)            
            ...
            obj.img_names = textscan(fopen([obj.exp_dir 'ImageSets/Segmentation/' obj.imgset '.txt']), '%s'); obj.img_names = obj.img_names{1};
                        
            % right now it's formatted for VOC
            VOCinit();
            obj.VOCopts = VOCopts;
            
%             ann = gason(fileread([obj.exp_dir 'annotations/instances_val2014.json']));
            global classids; classids = [
               ...
               %cell2mat({ann.categories.id});
            global classnames; classnames = { 'person',    'bicycle' ,   'car'  ,  'motorcycle'  ,  'airplane' ,   'bus'  ,  'train'  ,  'truck'  ,  'boat' ,   'traffic light'  ,  'fire hydrant'  ,  'stop sign'   , 'parking meter'   ,'bench'  ,  'bird', ...
               'cat'  ,  'dog'  ,  'horse'   , 'sheep'   , 'cow' ,   'elephant'  ,  'bear',    'zebra'  ,  'giraffe'  ,  'backpack'  ,  'umbrella' ,   'handbag',    'tie' ,   'suitcase' ,   'frisbee' ,   'skis', ...
 ...
 'vase' ,   'scissors'   , 'teddy bear'  ,  'hair drier'   , 'toothbrush'};
            ...
            if(~exist(obj.all_whole_ids_file, 'file'))
              disp('Computing and caching dataset meta-data (overlap_parts with GT, etc)...');
...
%                           for k=i:numel(obj.img_names)    , warning('removed parfor!');                      
                              ...
                              elseif(strcmp(obj.overlap_type, 'norm_intersection_parts'))
                                  SvmSegm_segment_quality_all_JL_parts(obj.img_names(k), obj.exp_dir, obj.mask_type, obj.overlap_type);
                              else
...
%                       end
                  ...
                  if length(Quality{1})==0, n_masks=0; disp('Empty image!'); else n_masks = numel(Quality{1}(1).q); end
...
%                 obj.img_name_to_id_hash = img_name_to_id_hash;
                ...
                save(obj.all_whole_ids_file, 'global_2_local_obj_ids', 'obj_2_img_ids', 'img_2_whole_ids', 'whole_2_img_ids', 'global_2_local_whole_ids', 'img_overlaps', '-V6');%'img_name_to_id_hash', 
            else
...
%               vgg_progressbar('feature loading', i/numel(un_img_ids), 5);
          ...
          if(exist(cache,'file'))
            ...
            for i=1:length(gtids)
              ...
              for j=1:numel(rec.objects)                
                ...
                Quality{i}(j).class = MSCOCO_classname_to_id(rec.objects(j).class);
        ...
        function [img_ids, obj_ids, Q] = collect_category_imgs(obj, category)   
            ...
            for i=1:numel(all_Quality)
                if(all_Quality(i).class == MSCOCO_classname_to_id(category))
        ...
        function [ap,class_fp, class_tp] = voc_cls_score(obj, local_ids, global_ids, labels, scores)                     
          ...
          warning('Only person class quality is evaluated !');
          for i=15
            class_scores = scores;
