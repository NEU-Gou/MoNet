function res = vl_mysimplenn(net, x, dzdy, res, masks, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'convt'
      res(i+1).x = vl_nnconvt(res(i).x, l.filters, l.biases, 'Upsample',l.upsample) ;
    case 'pool'
    ...
    case 'custom_masks'      
      if ~isempty(masks)
        res(i+1) = l.forward(l, res(i), res(i+1), masks) ;
      else
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
      ...
      case 'convt'
        [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
            vl_nnconvt(res(i).x, l.filters, l.biases, ...
                      res(i+1).dzdx, 'Upsample',l.upsample) ;
      case 'pool'
      ...
      case 'custom_masks'
        if ~isempty(masks)
          res(i) = l.backward(l, res(i), res(i+1), masks) ;
        else
          res(i) = l.backward(l, res(i), res(i+1)) ;
        end
      case 'custom_segmentation'
