function [ pre ] = nnlogmat_backward( layer, pre, now )
%nnsqrtmat_backward matrix wise log
%   now.x       - [1,1,c*c,n]
...
%         Dmin = min(find(ind, 1, 'last'), n+1);
        ...
        dLdV(:, 1:Dmin) = 2 *  symmetric(dLdC) * V(:, 1:Dmin) * ...
            diag(log(diag_S(1:Dmin)+ layer.eps));  
        dLdS(1:Dmin, 1:Dmin) =   diag((diag_S(1:Dmin) + layer.eps).^(-1)) * ...
